...
public class sPlayerController : MonoBehaviour
{
    ...
    [Header("Added to script by Josh")]
    private float lowGrav;
    private float lowJumpSpeed;
    [SerializeField] float highGrav;
    [SerializeField] float highJumpSpeed;
    private bool gravToggle = false;
    public void ToggleGrav()
    {
        ...
        {
            m_Gravity = highGrav;
            m_JumpSpeed = highJumpSpeed;
        }
        else
        {
            m_Gravity = lowGrav;
            m_JumpSpeed = lowJumpSpeed;
        }
    }
    ...
    private void Start()
    {
        // Added by Josh
        lowGrav = m_Gravity;
        lowJumpSpeed = m_JumpSpeed;
        EventController.OnGravityDisabled += ToggleGrav;
        EventController.OnGravityEnabled += ToggleGrav;
    ...
    private void Update()
    {
        ...
        if (m_ToggleRun && m_Grounded && Input.GetButtonDown("Run") && sUIManager.instance.xboxInputs)
        {
            m_Speed = (m_Speed == m_WalkSpeed ? m_RunSpeed : m_WalkSpeed);
            footsteps.pitch = (m_Speed == m_WalkSpeed ? 2f : 1f);
        }
    }
    ...
    private void FixedUpdate()
    {
        ...
        if (m_Grounded)
        {
            ...
            if (!m_ToggleRun)
            {
                m_Speed = Input.GetButton("Run") ? m_RunSpeed : m_WalkSpeed;
                footsteps.pitch = Input.GetButton("Run") ? 2f : 1f;
            }
